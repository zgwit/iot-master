...
func historyRouter(app *gin.RouterGroup) {
	...
	app.POST("/search", curd.ApiSearch[model.History]())

	app.GET("/list", curd.ApiList[model.History]())

	app.GET("/:id", curd.ParseParamId, curd.ApiGet[model.History]())

	app.GET("/:id/delete", curd.ParseParamId, curd.ApiDelete[model.History](nil, nil))
