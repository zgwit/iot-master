...
func (s *Simatic) Read(station int, addr protocol.Addr, size int) ([]byte, error) {
	...
	bin.WriteUint16(buf[6:], uint16(size))    // 访问数据的个数
	bin.WriteUint16(buf[8:], address.DB)      //db number DB块编号，如果访问的是DB块的话
	buf[10] = address.Code                    //area 访问数据类型
	bin.WriteUint24(buf[11:], address.Offset) //address 偏移位置
...
func (s *Simatic) Write(station int, addr protocol.Addr, data []byte) error {
	...
	bin.WriteUint16(buf[6:], uint16(length))  // 访问数据的个数
	bin.WriteUint16(buf[8:], address.DB)      // DB块编号，如果访问的是DB块的话
	buf[10] = address.Code                    // 访问数据类型
	bin.WriteUint24(buf[11:], address.Offset) // 偏移位置
	// 按字写入
	buf[14] = 0x00
	buf[15] = 0x04
	bin.WriteUint16(buf[16:], uint16(length*8)) // 按位计算的长度
...
func packCommand(cmd []byte) []byte {
	...
	buf[0] = 0x03
	buf[1] = 0x00
	bin.WriteUint16(buf[2:], uint16(length+17)) // 长度
	//ISO-COTP
	buf[4] = 0x02 // 固定
	buf[5] = 0xF0
	buf[6] = 0x80
	...
	buf[9] = 0x0  //Reserved
	buf[10] = 0x0
	bin.WriteUint16LittleEndian(buf[11:], 0)  // PDU ref 标识序列号(可以像Modbus TCP一样使用)
	bin.WriteUint16(buf[13:], uint16(length)) // Param length
	bin.WriteUint16(buf[15:], 0)              // Data length
