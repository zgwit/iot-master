...
func (t *A3C1) BuildCommand(cmd []byte) []byte {
	...
	buf[1] = 0x46                              //F 帧识别号H
	buf[2] = 0x39                              //9 帧识别号L
	bin.WriteByteHex(buf[3:], t.StationNumber) //站编号
	bin.WriteByteHex(buf[5:], t.NetworkNumber) //网络编号
	bin.WriteByteHex(buf[7:], t.PlcNumber)     //PLC编号
	bin.WriteByteHex(buf[9:], t.UpperNumber)   //上位机编号
	...
	//最后两位是校验
	bin.WriteByteHex(buf[len(buf)-2:], sum)
