...
func (t *FxSpecial) Read(address string, length int) ([]byte, error) {
	...
	buf := make([]byte, 23)
	buf[0] = 0x05                               //ENQ
	bin.WriteUint8Hex(buf[1:], t.StationNumber) //站号
	bin.WriteByteHex(buf[3:], t.PlcNumber)      //PLC号
	if addr.IsBit {
		buf[10] = 'B' //位
	} else {
		buf[10] = 'W'                //字
	...
	buf[11] = 'R'                               //读取
	bin.WriteUint8Hex(buf[12:], t.Delay)        //延迟
	copy(buf[14:], addr.Address)                // 偏移地址
	copy(buf[19:], fmt.Sprintf("%02d", length)) //读取长度

	if t.CheckSum {
		//最后两位是和校验
		bin.WriteUint8Hex(buf[len(buf)-2:], bin.Sum(buf[1:len(buf)-2]))
	} else {
		buf = buf[:21]
	}
