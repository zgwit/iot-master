...
func (t *A3EAdapter) BuildCommand(cmd []byte) []byte {
	...
	bin.WriteByteHex(buf[4:], t.NetworkNumber) //网络编号
	bin.WriteByteHex(buf[6:], t.PlcNumber)     //PLC编号
	bin.WriteByteHex(buf[8:], t.IoNumber)      //目标模块IO编号
	bin.WriteByteHex(buf[10:], 0xFF)
	bin.WriteByteHex(buf[12:], t.StationNumber)    //站编号
	bin.WriteUint16Hex(buf[14:], uint16(length+4)) // 请求数据长度
